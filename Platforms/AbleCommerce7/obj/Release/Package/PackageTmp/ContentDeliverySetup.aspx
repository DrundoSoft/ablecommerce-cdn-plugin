...
<script runat="server">
    ...
    private const string _ProductVersion = "1.0.0.103012";
    private const string _ProductUrl = "http://cdn.drundo.com/products/ablecommerce/drundocdn/";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            _AcVersion = AbleCommerceVersion.Instance.Version;
    ...
    private void UpdateSiteAdminMenu(bool configure)
    {
        ...
        sitemap.Load(appRootFolder + "admin\\menu.sitemap");
        XmlNamespaceManager manager = new XmlNamespaceManager(sitemap.NameTable);
        manager.AddNamespace("s", sitemap.DocumentElement.NamespaceURI);
        XmlNode menuConfig = sitemap.SelectSingleNode("//s:siteMapNode[@title = 'Configure']", manager);
        ...
        if (!isConfigured && configure)
        {
            XmlNode menuSection = sitemap.CreateElement("siteMapNode");
            ...
            sitemap.Save(appRootFolder + "admin\\menu.sitemap");
        }
    ...
    private void UpdateSiteAdminBreadcrumbsMenu(bool configure)
    {
        ...
        XmlDocument sitemap = new XmlDocument();
        sitemap.Load(appRootFolder + "admin\\breadcrumbs.sitemap");
        XmlNamespaceManager manager = new XmlNamespaceManager(sitemap.NameTable);
        manager.AddNamespace("s", sitemap.DocumentElement.NamespaceURI);
        XmlNode menuConfig = sitemap.SelectSingleNode("//s:siteMapNode[@title = 'Configure']", manager);
        ...
        if (!isConfigured && configure)
        {
            ...
            menuSection.Attributes.Append(attrUrl);
            menuConfig.AppendChild(menuSection);
            sitemap.Save(appRootFolder + "admin\\breadcrumbs.sitemap");
        }
